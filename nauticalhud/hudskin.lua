local SKINID = "NAUTICAL"
local HUDATLAS = "images/nautical_hud.xml"
if not TUNING.HUDSKIN then
    ...
    local function InstallImageLib()
        ...
        Image.SetTexture = function(self, atlas, tex, ...)
            ...
            if rule and rule[atlas] and rule[atlas][tex] then 
            atlas, tex = unpack(rule[atlas][tex]) 
            end
...
local function AddRules(ret)
    ...
    ret["images/lines_up.xml"] = {["lines_up.tex"] = "crafting_inventory_arrow_l_idle.tex"}
    ret["images/lines_down.xml"] = {["lines_down.tex"] = "crafting_inventory_arrow_r_idle.tex"}
end
